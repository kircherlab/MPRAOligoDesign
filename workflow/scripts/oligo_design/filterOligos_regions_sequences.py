...
def cli(seqs, regions, map_in, maxHomLength, repeat, simple_repeats_file, tss_pos_file, ctcf_motif_file, map_out):
    ...
    if regions:
        failed["regions"], fail_reasons = regions_filter(regions, repeatIndex, TSSIndex, CTCFIndex, repeat)

    if seqs:
        failed["seqs"], reasons = seqs_filter(seqs, maxHomLength)
